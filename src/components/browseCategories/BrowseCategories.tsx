...
export default function BrowseCategories() {
    ...
    const categoryItems: CategoryItem[] = [
        { category: "Art", imgUrl: artImg, link: "<Link to Art category>", icon:ArtIcon},
        { category: "Collectibles", imgUrl: collectiblesImg, link: "<Link to Collectibles category>" ,icon:CollectionIcon},
        { category: "Music", imgUrl: musicImg, link: "<Link to Music category>",icon:MusicIcon },
        { category: "Photography", imgUrl: photographyImg, link: "<Link to Photography category>",icon:PhotographyIcon },
        { category: "Video", imgUrl: videoImg, link: "<Link to Video category>" ,icon:VideoIcon},
        { category: "Utility", imgUrl: utilityImg, link: "<Link to Utility category>" ,icon:UtilityIcon},
        { category: "Sport", imgUrl: sportImg, link: "<Link to Sport category>" ,icon:SportIcon},
        { category: "Virtual Worlds", imgUrl: virtualWorldsImg, link: "<Link to Virtual Worlds category>",icon:VirtualWordsIcon }
    ];
    ...
    return (
        <div className="px-[30px] py-[40px] md:px-[72px] lg:px-[115px] lg:py-[80px]">
         ...
         <div key={index} className="w-full max-w-[147.5px] h-[209px] md:max-w-[150px] lg:max-w-[240px]  lg:h-[316px] rounded-t-[20px]">
         <div className="relative max-h-[142px] lg:max-h-[240px] h-full w-full rounded-t-[20px] overflow-hidden">
                <div className="absolute inset-0 z-10 bg-cover bg-center rounded-t-[20px]" style={{ backgroundImage: `url(${item.imgUrl})` }}>
                  <div className="absolute inset-0 backdrop-blur-sm rounded-t-[20px]"></div>
                </div>
                <img src={item.imgUrl} alt={item.category} className="relative z-0 h-full w-full object-cover rounded-t-[20px]" />
